<template>
  <div>
    ...
    <div class="container_game1">
      <div v-if="isCorrect">
        <div @click="handleRightQuestionpt2()" id="displayWrongRight"></div>
        <img
          id="correctbutton"
      ...
      <div v-if="isWrong">
        <div @click="handleWrongQuestionpt2()" id="displayWrongRight"></div>
        ...
        <b-row align-h="center" align-v="center" class="containerRow01">
          <b-col cols="3">
            <img
              ...
              id="gamefoto1"
              :src="images[0].url"
              @click.prevent="verifyAnswer($event)"
            />
...
export default {
  ...
  computed: {
    ...mapGetters(["getCategories", "getLoggedUser"]),
  },
  methods: {
    ...mapMutations([
      "INCREMENT_TOTAL_ANSWERS",
    ...
    getQuestion() {
      ...
      this.roll = Math.round(Math.random() * 6);
      this.roll2 = Math.round(Math.random() * 6);

     /*  let flag = true;

      while (flag) {
        if (this.aux.includes(this.roll)) {
 ...
 */
    ...
    getQuestionGroup() {
      ...
      console.log(this.images[2].url);
      console.log(this.images[3].url);
    ...
    verifyAnswer(event) {
      ...
      this.verifyIfBadgeisObtainable()
    },
    handleRightQuestion() {
      this.isCorrect = true;
    },
    ...
    handleWrongQuestionpt2() {
     ...
     verifyIfBadgeisObtainable() {
      for(let i=0; i<this.getLoggedUser.categories.length-1; i++){
        if (this.getLoggedUser.categories[i].correct == 10 || this.getLoggedUser.categories[i].correct == 25 || this.getLoggedUser.categories[i].correct == 50 ){
          this.UPDATE_BADGE(i);
